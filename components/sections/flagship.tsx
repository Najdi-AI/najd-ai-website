...
export function Flagship() {
  ...
  return (
    ...
    >
      {/* Ambient brand glows */}
      <div className="pointer-events-none absolute -start-32 top-10 h-[30rem] w-[30rem] rounded-full bg-najd-blue/10 blur-[130px]" />
      <div className="pointer-events-none absolute -end-32 bottom-0 h-[28rem] w-[28rem] rounded-full bg-najd-blue/10 blur-[130px]" />
      <div
        className="pointer-events-none absolute inset-0 [mask-image:radial-gradient(ellipse_70%_60%_at_50%_30%,black,transparent)]"
        style={{
          backgroundImage:
            "linear-gradient(to right, rgba(38,153,214,0.05) 1px, transparent 1px), linear-gradient(to bottom, rgba(38,153,214,0.05) 1px, transparent 1px)",
        ...
        <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-2 lg:gap-16">
          ...
          <div className="flex flex-col items-start gap-6 text-start">
            ...
            <Reveal delay={0.05} className="flex flex-col items-start gap-3">
              ...
              <span className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                <span className="font-display text-2xl font-bold tracking-tight text-gradient-najd sm:text-3xl">
                  {p.name}
                </span>
            ...
            <Reveal delay={0.1}>
              <span className="inline-flex items-center gap-2 rounded-full border border-najd-blue/25 bg-najd-blue/[0.07] px-3.5 py-1.5 text-[11px] font-semibold text-najd-blue sm:text-xs">
            ...
            <Reveal delay={0.18}>
              <p className="max-w-xl text-pretty text-[15px] leading-relaxed text-muted-foreground sm:text-base">
                <span className="font-semibold text-najd-blue">
                  {p.descriptor}
                </span>
                {" — "}
                {p.desc}
          ...
          <Reveal y={30} delay={0.12} className="flex items-center justify-center">
            <div className="relative w-full max-w-lg">
              <div className="pointer-events-none absolute inset-0 rounded-full bg-[radial-gradient(circle_at_50%_50%,rgba(38,153,214,0.16),transparent_65%)]" />
              ...
              <motion.li key={f.title} variants={staggerItem} className="h-full">
                <div className="relative h-full rounded-2xl border border-border/60 bg-white/[0.02] p-1.5">
                  ...
                  <div className="relative flex h-full flex-col gap-4 rounded-[14px] bg-card/40 p-6">
                    <span className="flex h-12 w-12 items-center justify-center rounded-xl border border-najd-blue/20 bg-najd-blue/[0.08] text-najd-blue">
          ...
          <Reveal delay={0.08} className="mt-10">
            <ol className="flex flex-col gap-3 md:flex-row md:items-stretch md:gap-0">
              {p.pipeline.map((step, i) => (
                ...
                >
                  <div className="flex h-full flex-1 flex-col gap-2 rounded-2xl border border-border/60 bg-white/[0.02] p-5 transition-colors hover:border-najd-blue/40">
                    <span className="flex h-8 w-8 items-center justify-center rounded-full bg-najd-blue/[0.12] font-display text-sm font-bold text-najd-blue">
                      {i + 1}
                    </span>
                  ...
                  {i !== p.pipeline.length - 1 && (
                    <span
                      aria-hidden="true"
                      className="flex shrink-0 items-center justify-center px-1 text-najd-blue/50 md:px-2"
                    >
                      <ArrowRight
                        className={cn(
