    ...
    <>
      ...
      <main className="mx-auto w-full max-w-7xl px-5 py-16 sm:px-8">
        <div className="flex flex-col items-start gap-4 border-b border-border/50 pb-12">
          <span className="inline-flex items-center gap-2 rounded-full border border-najd-green/25 bg-najd-green/[0.06] px-3.5 py-1.5 text-xs font-semibold text-najd-green">
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {items.map((c) => (
                  <Link
                    ...
                    className="group relative flex flex-col gap-2 overflow-hidden rounded-2xl border border-border/60 bg-white/[0.02] p-5 transition-all hover:-translate-y-1 hover:border-najd-green/40 hover:bg-white/[0.04]"
                  >
                    <div className="pointer-events-none absolute -right-10 -top-10 h-24 w-24 rounded-full bg-najd-green/10 opacity-0 blur-2xl transition-opacity group-hover:opacity-100" />
                    <div className="flex items-start justify-between gap-2">
                      <h3 className="font-display text-base font-semibold text-foreground">{c.title}</h3>
                      <ArrowUpRight className="h-4 w-4 shrink-0 text-muted-foreground transition-transform group-hover:-translate-y-0.5 group-hover:text-najd-green rtl:rotate-[-90deg]" />
                    </div>
                    <p className="text-sm leading-relaxed text-muted-foreground">{c.description}</p>
                    {c.usedOnSite && (
                      <span className="mt-1 inline-flex w-fit items-center gap-1.5 rounded-full bg-najd-green/10 px-2.5 py-1 text-[11px] font-medium text-najd-green">
