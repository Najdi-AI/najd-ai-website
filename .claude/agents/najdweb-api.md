...
## What exists
- `app/api/contact/route.ts` — the POST endpoint behind the contact form. It validates (`name` ≥ 2, email regex, `message` ≥ 5; `company`/`interest` captured but not required), returns `{ ok:false, errors:[...] }` (400) on bad body/validation and `{ ok:true }` on success. **Today it only `console.log`s the lead — no real delivery** — and its outer `catch {}` is bare (silent 500). API routes are NOT locale-prefixed and are exempt from `middleware.ts` (matcher excludes `/api`).
