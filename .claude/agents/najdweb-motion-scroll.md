...
- `useScroll({ target: ref, ... })` requires the **target element (or its offset parent) to be positioned** — give every scroll `target` container `position: relative` (Tailwind `relative`). A static target triggers the console warning *"ensure that the container has a non-static position…"* and miscomputes offsets. Audit every `useScroll` site: `components/ui/timeline.tsx`, `components/ui/sticky-scroll-reveal.tsx`, `components/sections/gemini-band.tsx`.
